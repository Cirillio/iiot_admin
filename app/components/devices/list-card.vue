...
import { binaryIcon, deleteIcon, deviceIcon, editIcon, gatewayIcon, sensorIcon } from "~/core/icons-map";
...
<template>
  <div class="group relative rounded-lg border border-default bg-elevated/20 hover:border-tertiary/50 hover:bg-elevated/50 transition-colors duration-100 ease-out overflow-hidden">

    <!-- Action buttons — appear on hover, stop navigation -->
    <div class="absolute top-3 right-3 z-10 flex gap-1 opacity-0 group-hover:opacity-100 transition-opacity duration-100">
      <UButton
        :icon="editIcon"
        size="xs"
        variant="solid"
    ...
    <NuxtLink :to="`/devices/${item.id}`" class="flex flex-col gap-4 p-5">
      ...
      <UIcon
        :name="deviceIcon"
        class="absolute -bottom-3 -right-3 size-24 pointer-events-none transition-opacity duration-100"
      ...
      <div class="flex items-center gap-2">
        ...
        <span class="ml-auto text-xs font-mono text-muted/50 pr-14">#{{ item.id }}</span>
      </div>
      ...
      <!-- Stats footer -->
      <div class="flex items-center gap-5 mt-auto pt-3 border-t border-default/60 text-sm">
